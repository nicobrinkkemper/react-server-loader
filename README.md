...
| `react-server-loader/webpack/runtime` | Runtime adapters for the webpack transport: `installWebpackGlobals` (back `__webpack_require__`/`__webpack_chunk_load__` with a closed registry, both dev/prod chunk-filename spellings), `gateModuleLoader` (sealed-gate-backed action loading), `createWebpackClient` (install-then-load, eval-order-safe). |
